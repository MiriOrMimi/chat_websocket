...
import 'package:socket_io_client/socket_io_client.dart' as io;  //messo minuscolo per risolvere un problema che non volevo avere
void main() {
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final _user = const types.User(id: '82ejf58-a484-4a89-ae75-a22bf8d6f3acertgertert',);
  ...
  void initState() {
    super.initState();
    socket = io.io("http://10.1.0.6:3000", <String, dynamic>{"transports": ["websocket"]});
    socket.on("connect", (_) {
    });
  ...
  void _loadMessages() async{
    ...
    final file = File(await _getFilePath());
    final messages = (jsonDecode(await file.readAsString()) as List).map((e) => types.Message.fromJson(e as Map<String, dynamic>)).toList();
  
  setState(() {_messaggi = messages;});
  }
  ...
  void _handleSendPressed(types.PartialText message) async{
    final textMessage = types.TextMessage(author: _user, id: const Uuid().v4(), text: message.text, createdAt: DateTime.now().millisecondsSinceEpoch);
  ...
  Future<void> addMessage(types.Message message) async{
    ...
    //Uri.parse("http://10.1.0.5:3000/submit"),
